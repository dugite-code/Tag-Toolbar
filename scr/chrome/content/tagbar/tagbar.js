	  ...
	  observe : function(aSubject, aTopic, aData) {
		  if (aTopic == 'nsPref:changed') {
			  ...
			  me.buildCatList();
				...
				var key = tags.shift();
				var cnt = aSubject.Count();
				for (var i=0; i<cnt; i++) {
					var msgFolder = aSubject.GetElementAt(i).QueryInterface(Components.interfaces.nsIMsgFolder);
...
TagToolbar.prototype.setTooltipForSavedCategory = function(aEvent)
{
	...
	if (aEvent.type == "mouseover") {
		...
		button.setAttribute("tooltip","tagtoolbar-searchcat-tip");
		var tipNodes = tip.firstChild.childNodes;
...
{	
	...
	var isLTR = true;
	try {
		isLTR=window.getComputedStyle(document.getElementById(this.mailBar),'').direction=='ltr';
	} catch(e) {
	}
/*	
