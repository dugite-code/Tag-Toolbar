    ...
    */
	...
	getCurrentTagColors: function() {
		...
		for (var i=0; i<tagArray.length; i++) {
			...
			if (this.bgColors[color] && ret.indexOf(this.bgColors[color]) == -1) ret.push(this.bgColors[color]);
		}
	...
	makeCSS: function(lightness) {
		...
		for (color in this.bgColors) {
			...
			var bgColor = this.calcBgColorBySaturation(rgb[0], rgb[1], rgb[2], lightness);
			var fgColor = this.getFgColor(rgb[0], rgb[1], rgb[2], bgColor[0], bgColor[1], bgColor[2], lightness);
			newCSS = newCSS + "treechildren::-moz-tree-cell-text(" + color + "), ." + color +":not([_moz-menuactive]) {color: rgb("+ fgColor.join(", ") + ") !important}\n";
			//newCSS = newCSS + "treechildren::-moz-tree-cell(" + color + "){background-color: rgba(" + this.bgColors[color] +", " + alpha +") !important;}\n";
			newCSS = newCSS + "treechildren::-moz-tree-cell(" + color + "){background-color: rgb(" + bgColor.join(", ") +") !important;}\n";
			//newCSS = newCSS + "treechildren::-moz-tree-cell(" + color + ", selected, focus){background-color: rgb(" + this.bgColors[color] +") !important;}\n";
			newCSS = newCSS + "treechildren::-moz-tree-cell(" + color + ", selected, focus){background-color: transparent !important;}\n";
		}
		newCSS = newCSS + "treechildren::-moz-tree-cell-text(lc-white, selected, focus){color: #FFFFFF !important}\n";
		newCSS = newCSS + "treechildren::-moz-tree-cell-text(lc-black, selected, focus){color: #000000 !important}\n";
		return newCSS;
	},
