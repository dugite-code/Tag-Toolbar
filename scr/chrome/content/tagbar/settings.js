...
TTBSetting.prototype.defaultOpt = function()
{
	...
	for (var i=0; i<elemNum; i++) {
		...
		if (localName == "textbox" && type == "int") {
			elem.value = defVal;
		} else if (localName == "listbox" && type == "string") {
...
TTBSetting.prototype.makeNewCat = function() {
	...
	var menuItem = this.addCatMenuItem(idCandidate, catName, "");
...
TTBSetting.prototype.swapTagsBetweenCats = function(fromList, toList) {
	...
	if (offset == -1) offset = toList.getRowCount();

	var tags = fromList.selectedItems;
	var sortFunc = function(a, b) {
...
TTBSetting.prototype.swapTagsInCat = function(list, up, tag)
{
	...
	var colorStyle = target.style.color;
	...
	if (target.getAttribute("catkey")) newTag.setAttribute("catkey", target.getAttribute("catkey"));
}
...
TTBSetting.prototype.endEditCatOrder = function()
{
	...
	for (var i = 0; i < rows; i++) {
		...
		menu.setAttribute("catkey", item.getAttribute("catkey"));
	}
